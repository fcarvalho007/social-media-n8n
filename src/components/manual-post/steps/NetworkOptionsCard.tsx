...
export const NetworkOptionsCard = forwardRef<NetworkOptionsCardHandle, NetworkOptionsCardProps>(function NetworkOptionsCard({
  ...
  const renderLinkedIn = () => (
    <div className="manual-group-stack">
      ...
      <div className="manual-field-stack"><Label className="manual-field-label">@mention</Label><div className="grid gap-2 sm:grid-cols-2"><Input className="manual-input-radius" ref={setFieldRef(fieldKey('linkedin', 'linkedinMention')) as React.Ref<HTMLInputElement>} value={mentionProfile} onChange={(e) => setMentionProfile(e.target.value)} placeholder="username ou URL do perfil" disabled={disabled} /><Input className="manual-input-radius" value={mentionName} onChange={(e) => setMentionName(e.target.value)} placeholder="nome a apresentar" disabled={disabled} /></div><Button type="button" size="sm" className="h-9" onClick={insertLinkedInMention} disabled={disabled}>Inserir</Button></div>
  ...
  const renderGoogleBusiness = () => <div className="manual-group-stack"><label className="flex items-center gap-2 text-sm"><Checkbox ref={setFieldRef(fieldKey('googlebusiness', 'googleBusinessCta')) as React.Ref<HTMLButtonElement>} checked={!!networkOptions.googlebusiness?.ctaEnabled} onCheckedChange={(checked) => updateNetwork('googlebusiness', { ctaEnabled: checked === true })} disabled={disabled} />Adicionar botão de ação (opcional)</label>{networkOptions.googlebusiness?.ctaEnabled && <div className="grid gap-2 sm:grid-cols-2"><Select value={networkOptions.googlebusiness?.ctaType ?? 'learn_more'} onValueChange={(value) => updateNetwork('googlebusiness', { ctaType: value as never })}><SelectTrigger className="manual-input-radius"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="book">Reservar</SelectItem><SelectItem value="order_online">Encomendar online</SelectItem><SelectItem value="buy">Comprar</SelectItem><SelectItem value="learn_more">Saber mais</SelectItem><SelectItem value="sign_up">Inscrever</SelectItem><SelectItem value="call_now">Ligar agora</SelectItem></SelectContent></Select>{networkOptions.googlebusiness?.ctaType !== 'call_now' && <Input className="manual-input-radius" value={networkOptions.googlebusiness?.ctaUrl ?? ''} onChange={(e) => updateNetwork('googlebusiness', { ctaUrl: e.target.value })} placeholder="https://..." disabled={disabled} />}</div>}</div>;
  ...
  return (
    <Card className="manual-card-shell manual-enter">
      ...
      <Dialog open={tagModalOpen} onOpenChange={setTagModalOpen}><DialogContent><DialogHeader><DialogTitle>Adicionar tag na fotografia</DialogTitle><DialogDescription>Define a pessoa e o slide onde a tag será aplicada.</DialogDescription></DialogHeader><div className="manual-group-stack"><div className="flex gap-2 rounded-md border border-primary/30 bg-primary/5 p-3 text-sm text-muted-foreground"><Info className="mt-0.5 h-4 w-4 shrink-0 text-primary" strokeWidth={1.5} />Nesta versão, a tag é adicionada automaticamente ao centro da imagem.</div><Input className="manual-input-radius" value={photoTagUsername} onChange={(e) => setPhotoTagUsername(e.target.value)} placeholder="@username" />{mediaPreviewUrls.length > 1 && <Select value={photoTagSlide} onValueChange={setPhotoTagSlide}><SelectTrigger className="manual-input-radius"><SelectValue /></SelectTrigger><SelectContent>{mediaPreviewUrls.map((_, index) => <SelectItem key={index} value={String(index)}>Slide {index + 1}</SelectItem>)}</SelectContent></Select>}<Button type="button" onClick={addPhotoTagAtCenter}>Adicionar no centro</Button></div></DialogContent></Dialog>
