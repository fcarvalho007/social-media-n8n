...
export const NetworkOptionsCard = forwardRef<NetworkOptionsCardHandle, NetworkOptionsCardProps>(function NetworkOptionsCard({
  ...
  // ── Resumo por rede (badges no header de cada sub-acordeão) ────────────
  const networkSummary = (network: SocialNetwork): string[] => {
    const tokens: string[] = [];
    if (network === 'instagram') {
      const opts = networkOptions.instagram ?? {};
      if (isStoryLinkSelected) {
  ...
  // Resumo agregado mostrado no estado `complete` da SectionCard
  const aggregatedSummary = useMemo(() => {
    const parts = selectedNetworks
      .map((network) => {
      ...
      .filter(Boolean) as Array<{ network: SocialNetwork; tokens: string[] }>;
